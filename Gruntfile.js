...
module.exports = function(grunt) {
        ...
        grunt.initConfig({
            ...
            grunt.registerTask('readJSON', 'Read JSON repeat fail test on jenkins', function(file) {
                // Force task into async mode and grab a handle to the "done" function.
                var done = this.async(),
                    fs = require('fs');
                file = './target/report/resultTest.json';
                ...
                function readFile(callback) {
                    fs.readFile(file, 'utf8', function(err, data) {
                        ...
                        for (var i in obj) {
                            ...
                            if (!obj[i].assertions[0].passed && obj[i].duration > 100) {
                                isError = true;
                                test = obj[i].description.match(/\d+/g);
                                id_string = id_string + test + '|';
                            }
                        }
                        if (isError) {
                ...
                async.waterfall([
                    ...
                    function(callback) {
                      if (isError) {
                        console.log(callback, id_string, isError);
                        grunt.task.run('exec:retry_jenkins:'+id_string);
